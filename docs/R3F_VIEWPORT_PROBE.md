...
- **Current R3F Canvas rendering: Failed physical verification.** The existing GL Layout Probe reports full logical R3F dimensions but its image remains a small left-aligned rectangle.
- **New R3F Viewport Probe: Implemented, not device-verified.** A physical comparison is still required.
...
This makes the smallest source-informed repair candidate precise: preserve or restore the native GL surface DPR when synchronizing the renderer. A later call that caps `WebGLRenderer.setPixelRatio()` to 1.35 can make Three.js configure a viewport substantially smaller than the native Expo GL surface. The probe tests that hypothesis without changing BowlScene.
...
- **Resync logical size:** applies the project's 1.35 capped DPR, synchronizes the renderer to measured logical dimensions, and resets bounds.
...
Do not describe the defect as fixed until the relevant result is **Device-verified by user**.
