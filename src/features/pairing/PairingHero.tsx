...
export function PairingHero({ height, waiting, environment, reducedMotion }: { height: number; waiting: boolean; environment: BowlEnvironment; reducedMotion: boolean }) {
  return <View style={[styles.hero, { height, backgroundColor: environment.backgroundEdge }]}><BowlScene pebbles={[]} previewPebbles={PAIRING_PREVIEW_PEBBLES} environment={environment} composition={waiting ? 'pairing-two' : 'pairing-single'} disabled reducedMotion={reducedMotion} onSend={async () => {}} onTouch={async () => {}} /></View>;
}
