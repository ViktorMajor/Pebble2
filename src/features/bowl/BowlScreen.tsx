...
export function BowlScreen({ pairId, connectionStatus, forceFallback, previewPebbles }: Props) {
  ...
  const [error,setError]=useState<string|null>(null); const [busy,setBusy]=useState(false); const [selectedPebbleId,setSelectedPebbleId]=useState<string|null>(null); const [emptyOpacity]=useState(()=>new Animated.Value(0));
  ...
  const visibleSelectedPebbleId=selectedPebbleId&&pebbles.some((pebble)=>pebble.id===selectedPebbleId)?selectedPebbleId:null;
  ...
  return <SafeAreaView edges={['left','right','bottom']} style={[styles.safe,{backgroundColor:environment.backgroundEdge}]}><StatusBar style="dark"/>
    <View style={styles.scene} accessibilityLabel={t('bowl.accessibility')}>
      {state.isLoading && !previewPebbles ? (
        ...
        <BowlScene pebbles={pebbles} environment={environment} disabled={closed || busy} reducedMotion={reducedMotion} forceFallback={forceFallback} selectedPebbleId={visibleSelectedPebbleId} onSelectedPebbleChange={setSelectedPebbleId} onSend={send} onTouch={touch} />
      ) : (
        <View style={styles.loading} />
      )}
      ...
      {!closed?<View pointerEvents="box-none" style={styles.a11yActions}>{pebbles.map((pebble)=>{const selected=selectedPebbleId===pebble.id;return <Pressable key={pebble.id} accessibilityRole="button" accessibilityLabel={pebble.incoming&&!pebble.touched?t('bowl.incoming'):t('bowl.held')} accessibilityHint={selected?t('bowl.sendAccessibilityHint'):t('bowl.selectAccessibilityHint')} accessibilityState={{selected}} accessibilityActions={[{name:selected?'send':'select',label:selected?t('bowl.sendAccessibilityAction'):t('bowl.selectAccessibilityAction')}]} onAccessibilityAction={(event)=>{if(event.nativeEvent.actionName==='select')setSelectedPebbleId(pebble.id);if(event.nativeEvent.actionName==='send'&&selected)void send(pebble.id);}} disabled={busy} delayLongPress={900} onLongPress={()=>{if(selected)void send(pebble.id);}} onPress={()=>{setSelectedPebbleId(pebble.id);if(pebble.incoming&&!pebble.touched&&pebble.transferEventId)void touch(pebble.transferEventId);}} style={styles.a11yAction}/>;})}</View>:null}
