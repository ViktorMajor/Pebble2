...
export function BowlScreen({ pairId, connectionStatus, forceFallback, previewPebbles }: Props) {
  ...
  return <SafeAreaView style={[styles.safe,{backgroundColor:environment.backgroundEdge}]}><StatusBar style="light"/>{!closed?<Link href="/(app)/settings" asChild><Pressable accessibilityRole="button" accessibilityLabel={t('app.settings')} hitSlop={8} style={styles.settings}><Text style={styles.settingsGlyph}>•••</Text></Pressable></Link>:null}
...
const styles=StyleSheet.create({safe:{flex:1},settings:{position:'absolute',right:spacing.md,top:spacing.xs,zIndex:10,minWidth:MIN_TOUCH_TARGET,minHeight:MIN_TOUCH_TARGET,alignItems:'center',justifyContent:'center'},settingsGlyph:{fontFamily:fonts.systemMedium,fontSize:17,letterSpacing:1.5,color:colors.textSubdued},scene:{width:'100%',alignSelf:'stretch',flex:1,minHeight:380,position:'relative'},loading:{position:'absolute',top:0,right:0,bottom:0,left:0,alignItems:'center',justifyContent:'center',backgroundColor:colors.atmosphere},empty:{position:'absolute',left:'7%',right:'7%',top:'12%',...typography.relationalHero,color:colors.textPrimary,textAlign:'center'},a11yActions:{position:'absolute',width:1,height:1,opacity:.01},a11yAction:{width:1,height:1},hint:{position:'absolute',left:spacing.xl,right:spacing.xl,bottom:spacing.sm,...typography.functionalSecondary,color:colors.textSubdued,textAlign:'center'},closed:{position:'absolute',left:spacing.xl,right:spacing.xl,bottom:spacing.md,fontFamily:fonts.relationalMedium,fontSize:21,lineHeight:28,color:colors.textPrimary,textAlign:'center'},errorRow:{position:'absolute',left:spacing.lg,right:spacing.lg,bottom:spacing.sm,alignItems:'center'},error:{...typography.functionalSecondary,color:colors.error,textAlign:'center'},retry:{minHeight:MIN_TOUCH_TARGET,justifyContent:'center'},retryText:{...typography.functionalPrimary,color:colors.textPrimary}});
