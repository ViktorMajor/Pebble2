...
import { colors, fonts, MIN_TOUCH_TARGET, motion, spacing } from '../../design/tokens';
...
export function BowlScreen({ pairId, connectionStatus, forceFallback, previewPebbles }: Props) {
  ...
  useEffect(()=>{emptyOpacity.stopAnimation();emptyOpacity.setValue(0);if(pebbles.length!==0||state.isLoading)return;const animation=Animated.timing(emptyOpacity,{toValue:1,duration:reducedMotion?0:motion.caption,delay:reducedMotion?0:motion.settle,useNativeDriver:true});animation.start();return()=>animation.stop();},[emptyOpacity,pebbles.length,reducedMotion,state.isLoading]);
  ...
  return <SafeAreaView style={[styles.safe,{backgroundColor:environment.background}]}><StatusBar style="light"/><View style={styles.header}><Text style={styles.title}>{t('bowl.title')}</Text>{!closed?<Link href="/(app)/settings" accessibilityRole="button" accessibilityLabel={t('app.settings')} style={styles.settings}>{t('app.settings')}</Link>:null}</View>
    <View style={styles.scene} accessibilityLabel={t('bowl.accessibility')}>
      ...
      {!state.isLoading&&pebbles.length===0?<Animated.Text accessibilityLiveRegion="polite" style={[styles.empty,{opacity:emptyOpacity}]}>{t('bowl.empty')}</Animated.Text>:null}
...
const styles=StyleSheet.create({safe:{flex:1},header:{height:72,paddingHorizontal:spacing.lg,flexDirection:'row',alignItems:'center',justifyContent:'space-between',zIndex:2},title:{fontFamily:fonts.systemMedium,fontSize:17,color:colors.textPrimary,letterSpacing:.2},settings:{fontFamily:fonts.systemMedium,fontSize:14,color:colors.textSubdued,minHeight:MIN_TOUCH_TARGET,paddingVertical:14,textDecorationLine:'none'},scene:{flex:1,minHeight:380},loading:{flex:1,alignItems:'center',justifyContent:'center'},empty:{position:'absolute',left:spacing.xl,right:spacing.xl,bottom:spacing.md,fontFamily:fonts.relational,fontSize:23,lineHeight:32,color:colors.textPrimary,textAlign:'center'},a11yActions:{position:'absolute',width:1,height:1,opacity:.01},a11yAction:{width:1,height:1},hint:{height:48,fontFamily:fonts.system,fontSize:13,color:colors.textSubdued,textAlign:'center',paddingHorizontal:spacing.xl},closed:{height:48,fontFamily:fonts.relational,fontSize:17,color:colors.textSubdued,textAlign:'center'},errorRow:{paddingHorizontal:spacing.lg,paddingBottom:spacing.md,alignItems:'center'},error:{fontFamily:fonts.system,color:colors.error,fontSize:14,textAlign:'center'},retry:{minHeight:MIN_TOUCH_TARGET,justifyContent:'center'},retryText:{fontFamily:fonts.systemMedium,color:colors.textPrimary}});
