...
export default function BowlRoute(){const state=useAppSession();const{t}=useI18n();if(state.isLoading)return<AppLoadingScreen/>;if(state.connectionErrorText)return<AppRetryScreen message={t('app.connectionError')} onRetry={()=>void state.connectionRefresh()}/>;if(!state.session)return<Redirect href="/(auth)"/>;if(!state.connectionId)return<Redirect href="/(app)/pairing"/>;return<BowlScreen pairId={state.connectionId} connectionStatus="active"/>;}
