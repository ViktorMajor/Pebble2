-- Qualify table columns that share names with RPC output parameters. This is a
-- forward-only correction for local databases that already applied Phase 17.
...
declare
  ...
  existing_request private.pebble_send_requests%rowtype; new_event public.pebbles%rowtype;
begin
  ...
  if existing_request.id is not null then
    ...
    select events.id,events.pair_pebble_id,events.pair_id,events.created_at into transfer_event_id,pair_pebble_id,pair_id,created_at
    from public.pebbles events where events.id=existing_request.pebble_id;
    return next; return;
  end if;
  select identities.* into selected_pebble from public.pair_pebbles identities
  join public.pairs selected_pair on selected_pair.id=identities.pair_id
  where identities.id=selected_pair_pebble_id and identities.current_holder_id=current_user_id
    and selected_pair.status='active' and private.is_pair_member(identities.pair_id,current_user_id)
  for update of identities;
  if selected_pebble.id is null then raise exception 'Pebble unavailable.'; end if;
  perform 1 from public.pairs selected_pair where selected_pair.id=selected_pebble.pair_id and selected_pair.status='active' for update;
  if not found then raise exception 'This connection is closed.'; end if;
  select members.user_id into partner_user_id from public.pair_members members
  where members.pair_id=selected_pebble.pair_id and members.user_id<>current_user_id limit 1;
  ...
  update public.pair_pebbles identities set current_holder_id=partner_user_id where identities.id=selected_pebble.id;
