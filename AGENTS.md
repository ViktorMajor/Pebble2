...
This repository has a premium Bowl experience backed by Supabase: minimal authentication, secure server-controlled connection pairing, exactly six persistent pair-owned pebble identities with an initial 3–3 distribution, atomic holder transfers, recipient-only irreversible touches, pair-scoped Realtime synchronization, server-side Expo push delivery, explicit connection/account lifecycle controls, local calendar-driven environmental light, a native 3D renderer with a 2D fallback, and an iOS ambient widget. There are no inactivity reminders or notification engagement mechanics.
